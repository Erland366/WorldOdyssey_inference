...
def _ensure_column_major_weight(weight: torch.Tensor) -> torch.Tensor:
    """Return a column-major 2D FP8 weight for sgl-kernel scaled GEMM.

    ModelOpt FP8 loading stores quantized linear weights as a transposed
    non-contiguous view, which gives sgl-kernel the column-major layout it
    expects. SGLang layerwise offload materializes those weights from a flat
    CPU buffer with normal row-major strides, so repair the layout after
    prefetch and before the GEMM.
    """

    if weight.ndim != 2 or weight.stride(0) == 1:
        return weight
...
class ModelOptFp8LinearMethod(LinearMethodBase):
    ...
    def apply(
        ...
        weight = layer.weight
        if self.cutlass_fp8_supported:
            weight = _ensure_column_major_weight(weight)
            if weight is not layer.weight:
